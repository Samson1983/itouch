...
public class ToggleButton2Preference extends Preference  {
	...
	protected View onCreateView(ViewGroup parent) {
		...
		subLayout.setOrientation(LinearLayout.VERTICAL);
		 if ("true".equals(AdHelper.getOnLineADVaule(getContext()))){
			 // 互动广告调用方式
			 subLayout.addView(new TextView(getContext()));
		 }
		...
		oneBbtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (oneBbtn.isChecked()){
					System.out.println("----onClick-------ON");
					//会执行onClick-->first
//					 ZhuoyuUIUtil.startServiceAll(getContext(),false);//启动所有服务
				}else{
					System.out.println("----onClick-------OFF");
					ZhuoyuUIUtil.stopServiceAll(getContext());//停止所有服务,关闭程序
					
				}
			}
		});
		oneBbtn.setChecked(oldValue);
		oneBbtn.setText(oneBbtn.isChecked()? "ON":"OFF"); //统一用英文
		...
		twoBbtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (Constant.autoSvrFlag){
					AppVersionAdapter.startMainActivity(getContext());					
				} 
